...
const Page = () => {
  ...
  const { startUpload, isUploading } = useUploadThing('imageUploader', {
    ...
    onUploadProgress(p) {
      // p is a number that is automatically updated by the library
      setUploadProgress(p) // set the progress state to the number
    },
  })

  // handle rejected and accepted user input
          ...
          {({ getRootProps, getInputProps }) => (
            ...
            >
              ...
              <div className='flex flex-col justify-center mb-2 text-sm text-zinc-700'>
                ...
                {/* above ternary states if uploading - show div, 
                if not, check if it is pending, then put that div, 
                if not pending, check if user is dragging over, \
                then show this span, else show the other */}
              </div>
              {isPending ? null : (
